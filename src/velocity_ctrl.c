...
float_t velCtrl_ISRStep(VelCtrlHandle *h, VelocityFilter *tracker){
    ...
    /* Dead zone: if velocity is very close to demand AND acceleration is
       small, go straight to the coast/bleed-off branch.  This prevents
       the braking check from chattering when the motor is essentially
       at the demanded velocity. */
    float_t v_deadzone = j_max * dt * dt;   /* ~0.00002 rad/s at 1kHz */
    float_t a_deadzone = j_max * dt;        /* ~0.02 rad/s^2 at 1kHz  */

    if (fabsf(v_error) < v_deadzone && fabsf(a) < a_deadzone){
        /* At demand velocity — bleed off any residual acceleration.
           Use exactly the jerk needed to zero acceleration in one tick. */
        ...
        if (must_brake){
        ...
        else if (fabsf(v_error) > v_deadzone){
            ...
            /* Near demand but acceleration is still significant — bleed off */
            j = -a / dt;
            if (j >  j_max) j =  j_max;
            if (j < -j_max) j = -j_max;
        }
